...
package object dao {
 ...
 // val findAccountById:(Long => Option[Account with PrimaryKey[Long]]) = findAccountDef(defaultEntityManager)_

 // val findPersonById:(Long => Option[Person with PrimaryKey[Long]]) = findPersonDef(defaultEntityManager)_
}